...
| `OLLAMA_ENABLED` | `true` | Set `false` for rule-based mode without Ollama |
| `OLLAMA_FALLBACK_ON_ERROR` | `true` | Rule-based fallback if Ollama fails |
| `OLLAMA_TIMEOUT` / `OLLAMA_NUM_PREDICT` | `120` / `1024` | Timeout and token cap |

The UI shows **archetype scoring** (primary, confidence, overlays) and the active inference mode. See [docs/runbook-ollama.md](docs/runbook-ollama.md).
